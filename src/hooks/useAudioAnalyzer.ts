...
function validateAudio(sampleRate: number, totalSamples: number, duration: number, fileName: string, opts?: { skipDurationLimit?: boolean }) {
  ...
  if (!opts?.skipDurationLimit && duration > MAX_DURATION_SECS) {
...
export function useAudioAnalyzer(selectedProfileId: string = 'none') {
  ...
  const loadTrack = useCallback(async (file: File, trackType: 'user' | 'reference', opts?: { skipDurationLimit?: boolean }) => {
    if (!file) return;
    setState(prev => ({
      ...prev,
    ...
    try {
      ...
      validateAudio(sampleRate, totalSamples, duration, file.name, opts);
